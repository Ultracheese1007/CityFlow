-- =====================================================================
-- V2__constraints_and_lookups.sql
-- 集中添加：
--   1) 业务表的唯一约束（upsert 的自然键）
--   2) 字典/查找表的种子数据（tb_shop_type）
-- 这样 V4/V5 才能用 INSERT ... ON DUPLICATE KEY UPDATE 实现幂等合并。
-- =====================================================================
...
ALTER TABLE `tb_blog`
    ADD UNIQUE KEY `uk_blog_title_user_shop` (`title`, `user_id`, `shop_id`);
...
ALTER TABLE `tb_shop_type`
    ...
    ADD UNIQUE KEY `uk_shop_type_name` (`name`);


-- ---------- 2) 字典数据：tb_shop_type ----------
-- 用 INSERT ... ON DUPLICATE KEY UPDATE 让脚本可重入
